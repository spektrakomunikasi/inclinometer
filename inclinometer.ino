...
alignas(Adafruit_ILI9341) uint8_t tftStorage[sizeof(Adafruit_ILI9341)];
...
void setup() {
  ...
  Serial.println("AP credentials and admin token loaded.");
...
void initDisplay() {
  if (tft == nullptr) {
    tft = new (tftStorage) Adafruit_ILI9341(&tftSPI, pins.tftDC, pins.tftCS, pins.tftRST);
  }
